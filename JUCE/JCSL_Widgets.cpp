...
void AudioWaveformDisplay::paint (Graphics& g) {
...
void AudioSpectrumDisplay::paint (Graphics& g) {
	float realV, maxV;
	g.fillAll (Colours::black);
...
//		printf("Paint %d @ %d (%g)\n", wid, hite, xScale);
		...
		for (unsigned x = 0; x < wid; x++) {			// window loop
			...
			for (unsigned y = 0; y < hite; y++) {			// frame loop
				g.setColour (Colour(circularBuffer[y] / maxV, 1.0f, 1.0f, 1.0f));	// HSVA
				unsigned yVal = hite - y;
				unsigned xVal = x * xScale;
				for (unsigned i = 0; i < xScale; i++) {		// draw loop
					g.setPixel(xVal+i, yVal);
				}
			}
		}
...
VUMeter::VUMeter() :
			...
			m_segments(32),
			m_markerWidth(2),
			m_minColour(Colours::green), 
			m_thresholdColour(Colours::yellow), 
			m_maxColour(Colours::red), 
			m_backgroundColour(Colours::black), 
			m_threshold(0.707f),
			m_value(0), 
			m_skew(1.0f),
			m_inset(0), 
...
void VUMeter::setColours(Colour& min, Colour& threshold, Colour& max, Colour& back) { 
	m_minColour = min; 
