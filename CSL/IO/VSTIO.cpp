...
VSTIO::	VSTIO(audioMasterCallback audioMaster, 
	...
	setUniqueID ('Filter');		// identify
	canProcessReplacing ();		// supports replacing output
	
	mInOut = new InOut(this, 2, 2, kNoProc);		// stereo i/o
...
bool VSTIO::getEffectName (char * name) { 
	vst_strncpy(name, "Freq", kVstMaxEffectNameLen);
	return true;
}
...
bool VSTIO::getProductString (char * text) { 
	vst_strncpy(text, "Filter", kVstMaxProductStrLen);
	return true;
}
...
Buffer & VSTIO::getInput(unsigned numFrames, unsigned numChannels) throw(CException) {
	if (mNumInChannels == 0)
